...
use warnings;
#Usage:
#perl .pl <IN bin_dir> <IN number_of_threads> <IN path_to_checkm2_dmnd> <IN path_to_GUNC_dmnd>
#this script needs checkm2 env (conda cerate -n checkm2) and gunc env (conda cerate -n gunc)

($dir, $cpu, $checkm2_db, $gunc_db) = @ARGV;
...
print OU2 "checkm2 predict --threads $cpu --input $dir --database_path $checkm2_db -x fa --output-directory s5_checkm2_out/ \n";

#gunc
print OU2 "conda activate gunc \n";
print OU2 "echo \"########running gunc########\"\n";
print OU2 "mkdir s5_gunc_out \n";
print OU2 "gunc run -t $cpu --input_dir $dir --db_file $gunc_db -o s5_gunc_out \n";
